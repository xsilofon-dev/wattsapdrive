# Contributors

WattSapDrive зібраний людьми й агентами разом.

| Хто | Роль | GitHub |
|-----|------|--------|
| **xsilofon-dev** | власник / нагляд | [@xsilofon-dev](https://github.com/xsilofon-dev) |
| **Cursor Agent** | код, UI, фікси, GitHub | [@cursoragent](https://github.com/cursoragent) |
| **DeepSeek** | модель / агент у Kilo | [@deepseek-ai](https://github.com/deepseek-ai) |
| **Kilo Code** | середовище агента (Kilocode) | [@Kilo-Org](https://github.com/Kilo-Org) |
- Cursor Agent: web UI, upload progress, GitHub landing, fixes
